...
class OptVerification extends React.Component{
    ...
    resendOTP(event){
        ...
        this.timer(event);
        axios.post('http://localhost:1234/users/create', this.props.location.state.details)
        .then((data)=>{
    ...
    submitOtp(event){
        ...
        if(this.state.otp===document.getElementById('otptext').value){
            axios.post('http://localhost:1234/users/otpVerified',{details:this.state.details})
